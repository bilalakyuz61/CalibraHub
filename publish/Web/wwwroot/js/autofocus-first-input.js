    ...
    function run() {
        ...
        if (input) {
            try {
                ...
                // Sadece BOS input'larda OR data-select-on-focus="true" attribute varsa select et.
                // Edit formlarinda mevcut degeri secmek kullaniciyi rahatsiz eder (beyaz parlama hissi
                // + tek tusla veri kaybetme riski). Bos input'ta select zaten gorunmez (deger yok).
                var shouldSelect = input.dataset && input.dataset.selectOnFocus === 'true';
                if (shouldSelect && typeof input.select === 'function'
