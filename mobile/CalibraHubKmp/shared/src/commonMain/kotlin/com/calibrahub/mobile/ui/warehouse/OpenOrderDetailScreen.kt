...
import com.calibrahub.mobile.data.OpenOrderDetailDto
...
/**
 * Açık Sipariş detayı → Teslim Et — CalibraHubAndroid `ui/warehouse/OpenOrderDetailScreen.kt` ile
 * BİREBİR sadık port. [docType] navigasyondan taşınır (OpenOrderDetailDto sözleşmesinde YOK —
 * [OpenOrderListScreen] zaten docType'a göre filtrelenmiş bir listeden geldiği için AppNavHost bu
 * değeri iki AYRI literal route (purchase/sales) üzerinden sabit geçirir).
 *
 ...
 * response'u, aynı bağlama-sonucu diyaloğu ([DeliveryLinkResultRow], DeliveryScreen'den `internal`
 * olarak PAYLAŞILDI).
 */
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun OpenOrderDetailScreen(session: SessionManager, orderId: Int, docType: DeliveryDocType, onBack: () -> Unit) {
            ...
            onSuccess = { d ->
                ...
                lineStates = d.lines.map { line ->
                    OpenOrderLineEditUi(
                        ...
                        quantityText = formatQty(line.openQuantity),
...
/** Açık sipariş kalemi düzenleme durumu — sunucudan gelen sabit bilgiler + kullanıcının doldurduğu
 * mutable alanlar. [copy] ile immutable güncelleme (updateLine() içinde). */
...
private fun OpenOrderLineCard(
    ...
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(modifier = Modifier.fillMaxWidth().padding(14.dp)) {
            ...
            Text(
                text = "Sipariş: ${formatQty(line.orderedQuantity)}$unitSuffix · " +
                    "Teslim: ${formatQty(line.deliveredQuantity)}$unitSuffix · " +
                    "Açık: ${formatQty(line.openQuantity)}$unitSuffix",
            ...
            if (qtyInvalid) {
                Spacer(Modifier.height(4.dp))
                Text(
                    text = "Miktar 0 ile ${formatQty(line.openQuantity)}$unitSuffix arasında olmalı.",
                    style = MaterialTheme.typography.labelSmall,
