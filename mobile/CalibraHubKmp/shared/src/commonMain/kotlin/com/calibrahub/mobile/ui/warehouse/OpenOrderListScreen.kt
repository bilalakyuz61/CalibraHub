...
import com.calibrahub.mobile.data.OpenOrderSummaryDto
import com.calibrahub.mobile.session.SessionManager
import kotlinx.coroutines.delay

/**
 * Açık Siparişler listesi — CalibraHubAndroid `ui/warehouse/OpenOrderListScreen.kt` ile BİREBİR
 * sadık port. [DeliveryScreen]'in aynı tek-composable-iki-yön deseni; arama/liste yapısı
 * debounce'lu server-side arama, reloadTick ile "Tekrar Dene", searching sırasında önceki liste
 * ekranda kalır.
 *
 * Satıra dokununca [onOpenDetail] ile sipariş Id'si taşınır (docType zaten çağıran tarafta bilinir
 * — bkz. AppNavHost'taki nav wiring: iki ayrı literal route PURCHASE/SALES için, tek ortak
 * `pendingOrderId` state'i ile detay ekranına aktarılır).
 */
    ...
    Card(onClick = onClick, modifier = Modifier.fillMaxWidth()) {
        Column(modifier = Modifier.fillMaxWidth().padding(14.dp)) {
            ...
            Row(
                ...
                Text(
                    text = "Açık miktar: ${formatQty(order.totalOpenQuantity)}",
                    style = MaterialTheme.typography.bodySmall,
                    color = MaterialTheme.colorScheme.onSurfaceVariant,
                )
            }
