...
/**
 * Cari (contact) rehberi (arama/seçim) — CalibraHubAndroid `ui/warehouse/ContactPickerField.kt`
 * ile BİREBİR sadık port. [MaterialPickerField]'ın BİREBİR deseni (300ms debounce, ≥2 karakter,
 * ExposedDropdownMenuBox, kod+ad gösterimi) ama contacts/search'e vurur ve barkod butonu YOK
 * (cari kartlarında barkod taraması anlamsız).
 *
 * MaterialPickerField'tan kasıtlı FARK: arama sonucu satırının kendisi ([ContactSearchDto]) zaten
 * tam kayıttır — malzeme akışındaki "sonuç listesinden seç → ayrı GET stock(code) ile çöz"
 * iki-aşamalı deseni burada YOK, seçim anında [onSelected] doğrudan çağrılır.
 *
