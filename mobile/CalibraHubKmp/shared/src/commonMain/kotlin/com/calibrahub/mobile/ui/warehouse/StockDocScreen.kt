...
import com.calibrahub.mobile.data.StockDocResult
 ...
 * SABİT kod (backend kontratı, CalibraHubAndroid ile BİREBİR AYNI). */
enum class StockDocMode(val widgetFormCode: String) {
    IN("STOCK_IN"),
    OUT("STOCK_OUT"),
}

/** Satır listesinin UI modeli. Sunucuya yalnız itemId + quantity gider (StockDocLineRequest);
 * kod/ad/birim satır kartında gösterim içindir. */
...
/**
 * Depo Giriş / Çıkış belge oluşturma ekranı — CalibraHubAndroid `ui/warehouse/StockDocScreen.kt`
 * ile BİREBİR sadık port (iki mod tek composable). [session] parametresi Android'in
 * `context.app.warehouseRepository`'sinin KMP karşılığıdır.
 *
 * Akış: lokasyon seç → paylaşılan [MaterialPickerField] ile malzeme ara/seç → miktar gir →
 * satıra ekle → (opsiyonel not) → Kaydet = POST stock-in|stock-out. Başarıda docNumber onay
 * diyaloğunda gösterilir; "Yeni Belge" formu temizleyip lokasyonu korur, "Kapat" geri döner.
 */
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun StockDocScreen(session: SessionManager, mode: StockDocMode, onBack: () -> Unit) {
...
@Composable
internal fun LocationSelectorCard(
    label: String,
...
/** Lokasyon listesi yüklenemedi kartı — hata mesajı + tekrar dene (stateless). TransferScreen/
 * CountScreen ile PAYLAŞILIR (`internal` — Android'de her dosyada ayrı `private` kopya olan bu
 * fonksiyon, [formatQty]'nin aksine burada gerçek DUPLICATE UI değil, tek kopyaya konsolide
 * edildi; CLAUDE.md DRY kuralı). */
@Composable
internal fun LocationsErrorCard(message: String, onRetry: () -> Unit) {
    Card(
        modifier = Modifier.fillMaxWidth(),
