using System.Globalization;

namespace CalibraHub.Application.Diagnostics;

/// <summary>
/// Hata log dosya deposu ayarları. Program.cs'de set edilir:
/// appsettings "Diagnostics:RootPath" ?? {ContentRoot}/App_Data/ErrorLogs,
/// "Diagnostics:ErrorLogRetentionDays" ?? 90 (0 = süresiz).
/// Audit'ten farkı: per-company klasörleme YOK — tek havuz, dosya-JSONL sunucu geneli
/// hata akışıdır (CompanyId alan olarak entry içinde tutulur, klasör anahtarı değildir).
/// </summary>
public sealed class SystemErrorLogOptions
{
    public required string RootPath { get; init; }

    /// <summary>Log dosyası saklama süresi (gün). 0 = süresiz sakla.</summary>
    public int RetentionDays { get; init; } = DefaultRetentionDays;

    public const int DefaultRetentionDays = 90;
}

/// <summary>
/// Günlük log dosyası yol/adlandırma kuralları — yazıcı ve okuyucu ortak kullanır.
/// Yapı: {root}/{yyyy-MM}/error-{yyyy-MM-dd}.jsonl
/// </summary>
public static class SystemErrorLogFileNaming
{
    public const string FilePrefix = "error-";
    public const string FileExtension = ".jsonl";

    public static string MonthDir(string root, DateTime dayUtc) =>
        Path.Combine(root, dayUtc.ToString("yyyy-MM", CultureInfo.InvariantCulture));

    public static string DayFile(string root, DateTime dayUtc) =>
        Path.Combine(
            MonthDir(root, dayUtc),
            FilePrefix + dayUtc.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture) + FileExtension);

    /// <summary>Dosya adından gün çıkarır ("error-2026-07-10.jsonl" → 2026-07-10). Uymayan ad → null.</summary>
    public static DateTime? ParseDay(string fileName)
    {
        if (!fileName.StartsWith(FilePrefix, StringComparison.OrdinalIgnoreCase) ||
            !fileName.EndsWith(FileExtension, StringComparison.OrdinalIgnoreCase))
            return null;
        var core = fileName.Substring(FilePrefix.Length,
            fileName.Length - FilePrefix.Length - FileExtension.Length);
        return DateTime.TryParseExact(core, "yyyy-MM-dd", CultureInfo.InvariantCulture,
            DateTimeStyles.AssumeUniversal | DateTimeStyles.AdjustToUniversal, out var day)
            ? day.Date
            : null;
    }

    /// <summary>
    /// [from..to] aralığındaki MEVCUT gün dosyalarını YENİDEN ESKİYE sıralı döner.
    /// Ay klasörleri üzerinden gezer; olmayan günler atlanır.
    /// </summary>
    public static IEnumerable<(DateTime Day, string Path)> EnumerateDayFilesDescending(
        string root, DateTime fromUtc, DateTime toUtc)
    {
        if (!Directory.Exists(root)) yield break;

        var from = fromUtc.Date;
        var to = toUtc.Date;

        foreach (var monthDir in Directory.EnumerateDirectories(root)
                     .OrderByDescending(d => Path.GetFileName(d), StringComparer.Ordinal))
        {
            foreach (var file in Directory.EnumerateFiles(monthDir, FilePrefix + "*" + FileExtension)
                         .OrderByDescending(f => Path.GetFileName(f), StringComparer.Ordinal))
            {
                var day = ParseDay(Path.GetFileName(file));
                if (day is null) continue;
                if (day.Value < from || day.Value > to) continue;
                yield return (day.Value, file);
            }
        }
    }
}
