...
public sealed class SqlSalesRepresentativeRepository : ISalesRepresentativeRepository
{
    ...
    public async Task<IReadOnlyCollection<SalesRepresentative>> GetAllAsync(CancellationToken cancellationToken)
    {
        ...
        command.CommandText = $"SELECT [id],[rep_code],[rep_name],[is_active],[created_at],[updated_at] FROM {_table} ORDER BY [rep_code];";
    ...
    public async Task<SalesRepresentative?> GetByIdAsync(int id, CancellationToken cancellationToken)
    {
        await using var connection = await _connectionFactory.OpenConnectionAsync(cancellationToken);
        await using var command = connection.CreateCommand();
        command.CommandText = $"SELECT [id],[rep_code],[rep_name],[is_active],[created_at],[updated_at] FROM {_table} WHERE [id] = @Id;";
    ...
    public async Task<int> AddAsync(SalesRepresentative entity, CancellationToken cancellationToken)
    {
        ...
        await using var command = connection.CreateCommand();
        command.CommandText = $"""
            INSERT INTO {_table} ([rep_code],[rep_name],[is_active],[created_at],[updated_at])
            VALUES (@Code, @Name, @IsActive, GETDATE(), GETDATE());
            SELECT CAST(SCOPE_IDENTITY() AS INT);
            """;
        command.Parameters.Add(new SqlParameter("@Code", entity.RepCode));
    ...
    public async Task UpdateAsync(SalesRepresentative entity, CancellationToken cancellationToken)
    {
        ...
        await using var command = connection.CreateCommand();
        command.CommandText = $"""
            UPDATE {_table} SET [rep_code]=@Code, [rep_name]=@Name, [is_active]=@IsActive, [updated_at]=GETDATE()
            WHERE [id] = @Id;
            """;
    ...
    {
        Id        = r.GetInt32(0),
        RepCode   = r.GetString(1),
        RepName   = r.GetString(2),
