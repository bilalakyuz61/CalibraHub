...
public sealed class SqlScheduledTaskRunRepository : IScheduledTaskRunRepository
{
    ...
    public async Task<int> CreateAsync(ScheduledTaskRun run, CancellationToken cancellationToken)
    {
        ...
        cmd.CommandText = $"""
            INSERT INTO {_table} ([task_id],[task_code],[started_at],[completed_at],[status],[message],[duration_ms],[trigger])
            VALUES (@TaskId,@TaskCode,@StartedAt,@CompletedAt,@Status,@Message,@DurationMs,@Trigger);
    ...
    public async Task CompleteAsync(int runId, int status, string? message, int durationMs, CancellationToken cancellationToken)
    {
        await using var connection = await _connectionFactory.OpenConnectionAsync(cancellationToken);
        await using var cmd = connection.CreateCommand();
        cmd.CommandText = $"""
    ...
    public async Task<IReadOnlyList<ScheduledTaskRun>> GetRecentByTaskAsync(int taskId, int limit, CancellationToken cancellationToken)
    {
        var list = new List<ScheduledTaskRun>();
        await using var connection = await _connectionFactory.OpenConnectionAsync(cancellationToken);
        await using var cmd = connection.CreateCommand();
        cmd.CommandText = $"""
            SELECT TOP (@Limit) [id],[task_id],[task_code],[started_at],[completed_at],[status],[message],[duration_ms],[trigger]
              FROM {_table}
    ...
    public async Task<IReadOnlyList<ScheduledTaskRun>> GetRecentByCodeAsync(string code, int limit, CancellationToken cancellationToken)
    {
        var list = new List<ScheduledTaskRun>();
        await using var connection = await _connectionFactory.OpenConnectionAsync(cancellationToken);
        await using var cmd = connection.CreateCommand();
