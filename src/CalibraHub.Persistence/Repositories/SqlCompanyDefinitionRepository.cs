...
public sealed class SqlCompanyDefinitionRepository : ICompanyDefinitionRepository
{
    private readonly SqlServerConnectionFactory _connectionFactory;
    private readonly string _tableName;

    public SqlCompanyDefinitionRepository(
    ...
    public async Task<IReadOnlyCollection<CompanyDefinition>> GetAllAsync(CancellationToken cancellationToken)
    {
        var companies = new List<CompanyDefinition>();
    ...
    public async Task<CompanyDefinition?> GetByIdAsync(int id, CancellationToken cancellationToken)
    {
        await using var connection = await _connectionFactory.OpenSystemConnectionAsync(cancellationToken);
        await using var command = connection.CreateCommand();
        command.CommandText = $"""
    ...
    public async Task<int> AddAsync(CompanyDefinition company, CancellationToken cancellationToken)
    {
        await using var connection = await _connectionFactory.OpenSystemConnectionAsync(cancellationToken);
        await using var command = connection.CreateCommand();
    ...
    public async Task UpdateAsync(CompanyDefinition company, CancellationToken cancellationToken)
    {
        await using var connection = await _connectionFactory.OpenSystemConnectionAsync(cancellationToken);
        await using var command = connection.CreateCommand();
        command.CommandText = $"""
    ...
    private static void AddInsertParameters(SqlCommand command, CompanyDefinition company)
    {
    ...
    private static CompanyDefinition MapCompany(SqlDataReader r)
    {
